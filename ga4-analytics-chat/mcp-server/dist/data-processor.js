export class DataProcessor {
    async processData(data, question, analysisType) {
    ...
    processSimpleQuery(data, question) {
    ...
    formatMetricResponse(metric, value, question) {
    ...
    processComparison(data, question) {
        const firstItem = data[0];
        const keys = Object.keys(firstItem);
        const metrics = keys.filter(key => typeof firstItem[key] === 'number');
        const dimensions = keys.filter(key => typeof firstItem[key] === 'string');
    ...
    processDeviceBreakdown(data, question) {
    ...
    selectRelevantMetric(question, availableMetrics) {
